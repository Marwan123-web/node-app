...
exports.addPost = async (req, res, next) => { //[]
    try {
        const {  author_id, author_name, body, date } = req.body
        const newPost = new Post({  author_id, author_name, body, date });
        await newPost.save();
        res.status(200).json({ msg: 'Post Added Successfuly', });
    } catch (error) {
        next(error)
    }
}
...
exports.deletePost = async (req, res, next) => { //[]
    try {
        ...
        let checkforPost = await Post.findOne({
            _id: postId
        });
        if (!checkforPost) {
            ...
            adminService.deletePost(postId).then((post) => {
                if (post) {
                    res.status(201).json({ msg: 'Post Deleted Successfuly' });
                }
            }).catch(err => {
                console.log(err);
...
exports.getPosts = async (req, res, next) => { //[]
    adminService.getPosts().then((posts) => {
        if (posts) {
            res.json(posts);
        } else {
...
exports.getmyPosts = async (req, res, next) => { //[]
    ...
    adminService.getmyPosts(userId).then((posts) => {
        if (posts) {
            res.json(posts);
        } else {
            res.status(404).json({ msg: 'Your Data Not Found' });
