...
// -------------------Add Material--------------------------
exports.addSemesterMaterial = async (req, res, next) => { //[]
    let materialType = req.body.type;
    let materialPath = req.body.path;
    let courseId = req.params.id;
    let semester_time = req.params.semester;
    try {
        let checkCourseId = await Course.findOne({
        ...
        let checkformaterial = await teacherService.searchformaterial(courseId, semester_time, materialType);
        if (!checkCourseId) {
            return res.status(400).json({

                msg: "course Not Found"
            });
        } else if (checkformaterial) {
            return res.status(400).json({

                msg: "this name of material was added before"
            });
        } else {
            teacherService.addMaterial(courseId, semester_time, materialType, materialPath).then((courseId) => {
                if (courseId) {
                    res.json({ msg: 'Material Added Successfuly' });
                }
            }).catch(err => {
                console.log(err);
                res.status(500).json({ msg: 'Internal Server Error' });
...
// -------------------Delete Semester material--------------------------
exports.deleteSemesterMaterial = async (req, res, next) => { //[]
    let code = req.params.id;
    ...
    try {
        ...
        if (!checkforcourse) {
            ...
            teacherService.deleteMaterial(code, semester_time, materialname).then((task) => {
                if (task) {
                    res.status(201).json({ msg: 'Material Deleted Successfuly' });
                }
            }).catch(err => {
                console.log(err);
                res.status(500).json({ msg: "Internal Server Error" });
